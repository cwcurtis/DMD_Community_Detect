...
def krn_dmd_cmp(gtot, nterms, thrshhld):
    nr, nc = gtot.shape
    gm = gtot[:, :nc - 1]
    gp = gtot[:, 1:]
...
def krn_dmd_cmp_alt(gtot, nterms, thrshhld):
    nr, nc = gtot.shape
    gm = gtot[:, :nc - 1]
    gp = gtot[:, 1:]
    amat = np.zeros((nc - 1, nc - 1), dtype=np.float64)
    ...
    q, sv, qh = np.linalg.svd(gmat, full_matrices=False)
    svm = np.max(sv)
    indskp = np.log10(sv / svm) > -thrshhld
    svd = sv[indskp]
    qrd = q[:, indskp]
    si = np.diag(1. / svd)
    kmat = qrd @ si @ qrd.T @ amat
    evls, evcs = np.linalg.eig(kmat)

    phim = gmat @ evcs
    kmodes = (np.linalg.solve(phim, gm.T)).T
